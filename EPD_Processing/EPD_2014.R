...
resource_name <- "EPD_201401" # For EPD resources are named EPD_YYYYMM
...
# 2014.
resource_name_list <- resources_table$name[grepl("2014", resources_table$name)]
...
EPD_2014$"0403_pc_laua" <- EPD_2014$total_quantity / EPD_2014$pop21  #creating per capita estimates of adps
EPD_2014 <- EPD_2014 %>%
  select(year_month,region,local_authority,pop21,total_quantity, "0403_pc_laua", rgn_pop21)

EPD_2014_Region <- EPD_2014 %>%
  group_by(region,rgn_pop21, year_month) %>%
  summarise(total_quantity = sum(total_quantity)) %>%
  rename(total_quantity_rgn = total_quantity)
EPD_2014_Region$"0403_pc_region" <- EPD_2014_Region$total_quantity_rgn / EPD_2014_Region$rgn_pop21
EPD_2014_Region <- EPD_2014_Region %>%
...
write.csv(EPD_2014, "EPD_0403_2014.csv", row.names = FALSE)
write.csv(EPD_2014_Region, "EPD_0403_2014_RGN.csv", row.names = FALSE)